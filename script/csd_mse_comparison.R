...
#######run 
# Params
#mode<-"subsampling"  # "subsampling"   "bootstrap"
#cor_mod<-"pearson"   #"spearmen"       "pearson"
#n_it<-20L           # number of iterations how many subsampling rounds are done
#n_sub<-20L          # for random subsampling , how many of the data is test data the rest is training data
...
#subsampling best results start with 20 iterations and 20 testdatasamplesize

#subsampling pearson

mode<-"subsampling"  
cor_mod<-"pearson"    #"spearmen"

n_it<-rep(c(10L,20L,30L,40L), each = 6)

n_sub<-10L

#subsampling spearmen
...
#bootstrap
if(mode == "bootstrap") cat(paste("Mode = ", mode, "Correlation method =  ",cor_mod,"Number of iterations", n_it, sep="\n"))
#subsampling
if(mode == "subsampling")cat(paste("Mode = ", mode, "Correlation method =  ",cor_mod,"Number of iterations", n_it,"Size of training set",n_sub, sep="\n"))
...
############################
############################
Rcpp::sourceCpp("script/welford_res.cpp")
Rcpp::sourceCpp("script/summarize_resultsp_res.cpp")
...
fast_spearman2 <- function(x) {
  ranks <- colRanks(x,ties.method = "average",preserveShape = TRUE)
  cor(ranks)
}
###### dont need a function
fast_pearson<-function(x){
...
run_cor_bootstrap_p_res <- function(x, n_it = n_it, nThreads = nThreads,
                                    verbose = TRUE, iterations_gap = 1L,sp="spearmen") {
  ...
  p_matrix <- array(0, c(n_res, n_res),
                    dimnames = list(gene_names, gene_names)
  )
  res_matrix <- array(0, c(n_res, n_res),
                      dimnames = list(gene_names, gene_names)
  )
  ### for p value calculation , reference correlation values from the complete data set
  if(sp == "spearmen") cor_matrix_compare<-fast_spearman2(x)
  if(sp == "pearson") cor_matrix_compare<-fast_pearson(x)
  ###
  for (i in seq_len(n_it)) {
    ...
    if(sp == "spearmen") cor_matrix <- fast_spearman2(x[bootstrap_ind, ])
    #cor_matrix <- fast_spearman(x[bootstrap_ind, ], nThreads)
    ##pearson
    if(sp == "pearson")cor_matrix <-fast_pearson(x[bootstrap_ind, ])
    
    welford_update_res(
      mu = rho_matrix, var = var_matrix,
      cor_matrix = cor_matrix,
      cor_matrix_compare=cor_matrix_compare,
  ...
  mean<-rho_matrix
  alt_mean<-0
  std_d<- sqrt(var_matrix)
  df<-n_it-1
  t_statistik<-(mean-alt_mean)/(std_d/sqrt(df))
  
  
  p_matrix<-2*pt(-abs(t_statistik),df)  
  
  
  #
  res_matrix<-res_matrix/n_it
  
  mse<-sum(res_matrix)/(n_res^2)#Mean Squared Error (MSE)
  
  list(rho = rho_matrix, var = var_matrix, p = p_matrix, res = res_matrix, mse = mse)
}
#####################

################################################################################
run_corr_subsampling_res<-function(x, n_it = n_it, nThreads =nThreads,n_sub = n_sub,
                                   verbose = TRUE, iterations_gap=1L,sp="spearmen"){
  blas_set_num_threads(nThreads)
  n_res <- ncol(x)
  gene_names <- colnames(x)
  ...
  ### for residual value calculation , reference correlation values from the training data(complete data without test data)
  ###
  for (i in seq_len(n_it)) {
    if (verbose && i %% iterations_gap == 0) {
      log_progress(glue("Running bootstrap iteration {i} of {n_it}..."))
    ...
    ###### change from bootstrap to random subsample
    bootstrap_ind <- sample(seq_len(nrow(x)),size = n_sub ,replace = FALSE)
    ######
    if(sp == "spearmen") {
      cor_matrix <- fast_spearman2(x[bootstrap_ind, ])          #testdata
      bootstrap_ind_a<-1:nrow(x)  #bootstrap_ind
      bootstrap_ind_a<-bootstrap_ind_a[!bootstrap_ind_a %in% bootstrap_ind]
      cor_matrix_compare<-fast_spearman2(x[bootstrap_ind_a, ])  #trainingsdata
    }
    ##pearson
    if(sp == "pearson"){
      cor_matrix <-fast_pearson(x[bootstrap_ind, ]) #testdata
      
      bootstrap_ind_a<-1:nrow(x)  #bootstrap_ind
      bootstrap_ind_a<-bootstrap_ind_a[!bootstrap_ind_a %in% bootstrap_ind]
      
      cor_matrix_compare<-fast_pearson(x[bootstrap_ind_a, ])  #trainingsdata
    }
    welford_update_res(
      mu = rho_matrix, var = var_matrix,
      cor_matrix = cor_matrix,
  ...
  p_matrix<-2*pt(-abs(t_statistik),df,lower.tail = FALSE)  
  ...
  #new list element single numeric value
  #all residues added together and divided by number of residues
  mse<-sum(res_matrix)/(n_res^2)#Mean Squared Error (MSE)
...
run_csd_p <- function(x_1, x_2, n_it = n_it, nThreads = nThreads,n_sub = n_sub,
                      verbose = TRUE, iterations_gap = 1L,sp="spearmen",version ="bootstrap") {
...
\t                  Number of threads: {nThreads}"))
  ...
  if(version == "bootstrap"){
    res_1 <- run_cor_bootstrap_p_res(x_1,
                                     n_it = n_it,
                                     nThreads = nThreads, verbose = verbose,
                                     iterations_gap = iterations_gap,sp="pearson"
    )
    if (verbose) {
      log_progress("Running correlation bootstrapping on second condition...")
    }
    res_2 <- run_cor_bootstrap_p_res(x_2,
                                     n_it = n_it,
                                     nThreads = nThreads, verbose = verbose,
                                     iterations_gap = iterations_gap,sp="pearson"
    )
    ...
    csd_df <- summarizeResultsp_res(res_1 = res_1, res_2 = res_2, n_threads = nThreads)
  }#bootstrap
  
  
  if(version == "subsampling"){
    
    res_1 <- run_corr_subsampling_res(x_1,
                                      n_it = n_it,
                                      nThreads = nThreads,n_sub = n_sub, verbose = verbose,
                                      iterations_gap = iterations_gap,sp="spearmen"
    )
    
    if (verbose) {
      log_progress("Running correlation subsampling on second condition...")
      
    }
    
    
    
    res_2 <- run_corr_subsampling_res(x_2,
                                      n_it = n_it,
                                      nThreads = nThreads,n_sub = n_sub, verbose = verbose,
                                      iterations_gap = iterations_gap,sp="spearmen"
    )
  ...
  print(paste0("MSE for the first Sample:  ", res_1["mse"]))
  print(paste0("MSE for the second Sample:  ",res_2["mse"]))
  ...
  list(csd = csd_df, mse_1 = res_1["mse"],mse_2 = res_2["mse"])
...
for(i in 1: length(n_it)){
  
  csd_results3<-run_csd_p(
    x_1 = Matrix_mayo_TCX_ctrl, x_2 = Matrix_mayo_TCX_ad,
    n_it = n_it[i], nThreads = 8L,n_sub = n_sub, verbose = FALSE,sp=cor_mod,version = mode
  )
  #csd_results3 <- csd_results3$csd
  mse_frame[i,1]=csd_results3$mse_1
  mse_frame[i,2]=csd_results3$mse_2
...
file1<- paste0("csd_results_",mode,"_",cor_mod,"_mse_",n_sub,".txt")
fwrite(mse_frame, file1 , append = FALSE)
