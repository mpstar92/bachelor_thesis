...
// [[Rcpp::plugins(openmp)]]
// [[Rcpp::export]]
DataFrame summarizeResultsp_res(List res_1, List res_2, int n_threads) {
  ...
  NumericMatrix p_val_1_mat = res_1["p"];
  NumericMatrix p_val_2_mat = res_2["p"];
  NumericMatrix res_1_mat = res_1["res"];
  NumericMatrix res_2_mat = res_2["res"];
  ...
  NumericVector p_val_1_vec (n_pairs);
  NumericVector p_val_2_vec (n_pairs);
  
  NumericVector res_1_vec (n_pairs);
  NumericVector res_2_vec (n_pairs);
...
#pragma omp parallel for simd num_threads(n_threads)
  ...
  for(R_xlen_t k = 0; k < n_pairs; k++){
    ...
    double p_val_1 = p_val_1_mat(i,j);
    p_val_1_vec[k] = p_val_1;
    double p_val_2 = p_val_2_mat(i,j);
    p_val_2_vec[k] = p_val_2;
    
    double res_1 = res_1_mat(i,j);
    res_1_vec[k] = res_1;
    double res_2 = res_2_mat(i,j);
    res_2_vec[k] = res_2;
  ...
  DataFrame res = DataFrame::create(Named("Gene1") = Gene1_vec, Named("Gene2") = Gene2_vec,
                                    ...
                                    Named("p_val_1") =p_val_1_vec,Named("p_val_2") = p_val_2_vec,
                                    Named("res_1") =res_1_vec,Named("res_2") = res_2_vec,
