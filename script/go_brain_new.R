...
#change input
csd_network<-data.table::fread("results/csd_results_subsampling_pearson_100_25.txt")


exit_file<- "sub_pear_100_25"
dir_1<-"results/sub_pear/"
...
csd_network <- csd_network %>% select(-(rho2:dVal))
...
merged_df1 <- merge(csd_network, brain_gold, by.x = c("Gene1", "Gene2"), by.y = c("V1", "V2"))
merged_df2 <- merge(csd_network, brain_gold, by.x = c("Gene2", "Gene1"), by.y = c("V1", "V2"))
